...
static void ADS1256_DelayDATA(void)
{
	...
	*/
	bsp_DelayUS(7);	/* The minimum time delay 6.5us */ //was 10
}
...
void ADS1256_ISR(void)
{
	if (g_tADS1256.ScanMode == 0)	/*  0  Single-ended input  8 channel£¬ 1 Differential input  4 channe */
	{
		...
		bsp_DelayUS(5); //took it out, set to one channel only

		ADS1256_WriteCmd(CMD_SYNC);
		bsp_DelayUS(5);

		ADS1256_WriteCmd(CMD_WAKEUP);
		bsp_DelayUS(5); //was 25
...
#define THRESHOLD		.05 //threshold for change detection
...
int  main()
{
   ...
   printf("ID=\r\n");  
	...
	//FILE *f = fopen("outfile4channel.txt", "w");	
		
		
	const int *int_buffer = malloc(currentsample * WINDOW_SIZE * sizeof(int));
		...
		while(1)
	{
		for (i = 0; i < WINDOW_SIZE; i++)
		{
			...
			adc[ACTIVE_CHANNEL] = ADS1256_GetAdc(ACTIVE_CHANNEL);
			volt[ACTIVE_CHANNEL] = (adc[ACTIVE_CHANNEL] * 100) / 167;
			buffer[i] = volt[ACTIVE_CHANNEL];
		...
		if ((current_max >= (old_max*(1+THRESHOLD))) || (current_max <= (old_max*(1-THRESHOLD))))
		{
			uint64_t timestamp = (unsigned)time(NULL);
			sendRequest(current_max,1,timestamp); //unsure if you wanted to send whole buffer or just the max
			//sendRequest(buffer,WINDOW_SIZE,timestamp);
		}
		
		old_max = current_max;
		current_max = 0;	//resets value for comparison in next iteration
	}
    bcm2835_spi_end();
    bcm2835_close();
