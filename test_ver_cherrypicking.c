...
#define THRESHOLD		.10 //threshold for change detection
...
int  main()
{
   ...
   printf("ID=\r\n");  
		...
		while(1)
	{
		...
		if ((volt[ACTIVE_CHANNEL] >= (current_val*(1+THRESHOLD))) || (volt[ACTIVE_CHANNEL] <= (current_val*(1-THRESHOLD))))
		{
			buffer[i] = volt[ACTIVE_CHANNEL];
			current_val = volt[ACTIVE_CHANNEL];
			i++;
		}
